 ...
 * 
 */
public class CnSegmentAdaptorImpl extends BaseAdaptor
		implements
		CnSegmentAdaptor {
	...
	protected String[] columns() {
		return new String[] { "cn_segment_id", "location.location_id",
				"location.location_chromosome", "location.location_start",
				"location.location_end", "cn_segment_mean",
				"cn_segment_markers", "study_id" };
	}
	...
	public int storeCnSegment(CnSegment segment, int studyId) {
		...
		try {
			...
			loc_query
					...
					.append(
							"(location_chromosome, " + "location_start, "
									+ "location_end) ")
					.append(" VALUES ")
					.append(
							"('" + segment.getLocation().getChromosome()
			...
			segment_query
					...
					.append(
							"(location_id, " + "cn_segment_mean, "
									+ "cn_segment_markers, " + "study_id)")
					.append(" VALUES ")
					.append(
							"('" + newLocId + "', '" + segment.getMean()
	...
	public Object createObject(ResultSet rs) {
		...
		try {
			if (rs.next()) {
				...
				segment =
						new CnSegment(id, loc, mean, numberOfMarkers, studyId);
			}
	...
	public CnSegment fetchCnSegmentById(int segmentId) {
		...
		try {
			...
			query
					...
					.append(
							" LEFT JOIN location ON cn_segment.location_id =" +
							" location.location_id")
					.append(" WHERE ")
					.append("cn_segment_id = " + segmentId);
	...
	private String getThresholdSQLClause(Double lowerTh, Double upperTh) {
		...
		if (lowerTh != null && upperTh != null) {
			qrystr =
					qrystr + " AND (cn_segment_mean < '" + lowerTh + "' AND "
	...
	@Override
	public Location fetchMaximalOverlappingCnSegmentRange(
			String chr,
			int start,
			int end,
			Double lowerTh,
		...
		query
				.append("SELECT ")
				.append(
						"location.location_id, MIN(location_start) " +
						"as minstart, MAX(location_end) as maxend")
				.append(" FROM ")
				.append(getPrimaryTableName())
				.append(
						" LEFT JOIN location ON cn_segment.location_id =" +
						" location.location_id")
				.append(
						" LEFT JOIN study ON study.study_id =" +
						" cn_segment.study_id")
				.append(
						" LEFT JOIN study_in_project ON study.study_id =" +
						" study_in_project.study_id")
				.append(
						" LEFT JOIN sample_on_platform ON " +
						"sample_on_platform_id = study_sample_on_platform_id")
				.append(
						" LEFT JOIN tissue_sample ON tissue_sample_id =" +
						" sample_on_platform_tissue_sample_id")
				.append(
						" LEFT JOIN organ ON organ_id =" +
	...
	@Override
	public CnSegment[] fetchCnSegments(
			String chr,
			int start,
			int end,
			Double lowerTh,
	...
	public void deleteCnSegment(CnSegment segment) {
		...
		try {
			...
			segment_query
					...
					.append(super.getPrimaryTableName())
					.append(" WHERE ")
					.append("cn_segment_id = " + segment.getId());
	...
	public void deleteCnSegment(int studyId) {
		...
		try {
			...
			loc_query
					...
					.append(" location")
					.append(
							" LEFT JOIN cn_segment ON cn_segment.location_id" +
							" = location.location_id")
					.append(" WHERE ")
					.append("study_id = " + studyId);
